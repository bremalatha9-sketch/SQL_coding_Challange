...
WHERE user_id = 2;
SELECT * FROM users
ORDER BY username;/* ORDER BY used to arrange the result*/

--  Challenge 2 — Posts Filter
--  Get posts posted BETWEEN '2024-01-01' AND '2024-12-31'.
SELECT * FROM posts
WHERE Posted_at BETWEEN '2024-01-01' AND '2024-12-31';/*BETWEEN is used check the value with in the range*/

--  Challenge 3 — NULL checks
--  Find posts that have NULL captions.
SELECT * FROM posts
WHERE caption IS  NULL; /* IS NULL is used to check if the value is empty*/
...
SELECT * FROM users
WHERE SUBSTRING_INDEX(email,'@',-1) /*substring_index is used to take a specific part of text*/
IN('gmail.com', 'hotmail.com', 'yahoo.com');/*IN- Choose from a list IN(membership id)*/
